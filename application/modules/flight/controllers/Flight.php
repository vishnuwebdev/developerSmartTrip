...
class Flight extends MX_Controller {
	function __construct() {
        ...
        $this->load->Model(array('Flight_Model','Common_Model','VisaModel'));
	...
	public function result() {	
		if ($this->input->get( "type" ) == "OneWay" || $this->input->get ( "type" ) == "Return"){
			...
			$data['preferred_currency'] = getCurrentCurrency();
			...
			// echo "Api URL = ".$this->url . '/Search/'."<br>";
			// echo "API Request Data : <br/>";
			// PrintArray($searchtypedata);
			// die;
			...
			if(isset($dsa_markup_temp) && !empty($dsa_markup_temp)){
				$dsa_markup = array();
				if (is_array($dsa_markup_temp)) {
					foreach ($dsa_markup_temp as $dsa_markup_temps) {
						if ($dsa_markup_temps->dsamark_markup_type == "all") {
							...
							$dsa_markup ['all'] ['dsamark_value'] = convertPrice($dsa_markup_temps->dsamark_value);
						} else {
							$dsa_markup [$dsa_markup_temps->dsamark_airline_code] ['dsamark_amount_type'] = $dsa_markup_temps->dsamark_amount_type;
							$dsa_markup [$dsa_markup_temps->dsamark_airline_code] ['dsamark_value'] = convertPrice($dsa_markup_temps->dsamark_value);
						}
					}
				}
	...
	function search_one_way($search_data) {
		...
		$search_data_tbo = array(
			...
			"PreferredAirlines" => NULL,  
			"PreferredCurrency" => $search_data['preferred_currency'],  			
			"Segments" => [ 
				array (
	...
	public function booking_detail() {
		...
		$nationality = $this->db->select('*')->get('nationality')->result();
		// ------------------------------- //
		$jsonResult = json_decode($_SESSION['flight'][$sessionid]['Search_Result_json']);
		$flightdataconfrim = $this->session->userdata("flight")[$sessionid]['farequote_data']['confirm_price']; 
		// MealDynamic and Baggage	
		$data_json = array (
			"EndUserIp" => $this->EndUserIp,
			"TokenId" => $this->TokenId,
			"TraceId" => $jsonResult->Response->TraceId,
			"ResultIndex"=>$flightdataconfrim->Response->Results->ResultIndex,
		);		
		...
		if ($this->session->userdata ( "flight" ) [$sessionid] ['search_RequestData'] ['type'] == "Return" && $isdomestic == "true") {
			...
			$dataJsonOB = array (
				...
				"TokenId" => $this->TokenId,
				"TraceId" => $jsonResult->Response->TraceId,
				"ResultIndex"=>$flightdataconfrimOB->Response->Results->ResultIndex,
			);
			$dataStringOB = json_encode ( $dataJsonOB );
 			$methodOB = "/SSR/";
 			$urlOB = $this->url . $methodOB;
			$chOB = curl_init ( $urlOB );
			curl_setopt ( $chOB, CURLOPT_CUSTOMREQUEST, "POST" );
			curl_setopt ( $chOB, CURLOPT_POSTFIELDS, $dataStringOB );
			curl_setopt ( $chOB, CURLOPT_RETURNTRANSFER, true );
			curl_setopt ( $chOB, CURLOPT_HTTPHEADER, array (
				'Content-Type: application/json',
				'Content-Length: ' . strlen ( $dataStringOB ) 
			) );
			$resultOB = curl_exec ( $chOB );
			// echo "outbond";
			// print_r($resultOB);
			// echo "==============================================";
			$dataJsonIB = array (
				...
				"TokenId" => $this->TokenId,
				"TraceId" => $jsonResult->Response->TraceId,
				"ResultIndex"=>$flightdataconfrimIB->Response->Results->ResultIndex,
			);
			$dataStringIB = json_encode ( $dataJsonIB );
 			$methodIB = "/SSR/";
 			$urlIB = $this->url . $methodIB;
			$chIB = curl_init ( $urlIB );
			curl_setopt ( $chIB, CURLOPT_CUSTOMREQUEST, "POST" );
			curl_setopt ( $chIB, CURLOPT_POSTFIELDS, $dataStringIB );
			curl_setopt ( $chIB, CURLOPT_RETURNTRANSFER, true );
			curl_setopt ( $chIB, CURLOPT_HTTPHEADER, array (
				'Content-Type: application/json',
				'Content-Length: ' . strlen ( $dataStringIB ) 
			) );
			$resultIB = curl_exec ( $chIB );
			$data ["confrimdataOB"] = $flightdataconfrimOB;
			$data ["confrimdataIB"] = $flightdataconfrimIB;
			$data ["baggageOB"] = json_decode($resultOB);
			$data ["baggageIB"] = json_decode($resultIB);
			...
			// $data ["allcountry"] = $allcountry;
			$flightdataconfrim = $this->session->userdata( "flight" )[$sessionid]['farequote_data']['confirm_price'];
			$baggageData = $this->Common_Model->get_table_order("*", "sessionId", $sessionid, "temp_baggage_data");
	...
	public function set_baggage_data(){
		$sessionid = $this->input->post ( 'sessionId' );

		$formData = array (
            "sessionId" => $this->input->post ( 'sessionId' ),
            "baggageWeight" => $this->input->post ( 'baggageWeight' ),
            "baggagePrice" => $this->input->post ( 'baggagePrice' ),
            "passenger"=> $this->input->post ( 'passenger' ),
            "type"=> $this->input->post ( 'type' ),
            "rawData"=> json_encode($this->input->post( 'rawData' )),
		...
		echo json_encode(array_sum($totalPrice));	
		exit;
	}


	public function get_baggage_data($sessionId){
		$baggageData = $this->Common_Model->get_table_order("*", "sessionId", $sessionId, "temp_baggage_data");
		echo json_encode($baggageData);	
		exit;
	}
	...
	public function set_meals_data(){
		...
		$this->db->where('type',$this->input->post ( 'type' ));
		$this->db->delete('temp_meals_data');
	   	$response = $this->Common_Model->insert_table ( "temp_meals_data", $formData );
	   	
	   	$whereMeal = array('sessionId' => $sessionid,'type'=>$this->input->post ( 'type' ));

	   	$mealsData = $this->Common_Model->get_all_record_where("*", $whereMeal, "temp_meals_data");
	   	$totalPrice = [];


	   	for($i=0; $i<=count($mealsData); $i++){
	   		$totalPrice[] = $mealsData[$i]->price;
	   	}
	...
	public function save_return_customer_data(){
		$sessionid = $this->input->post ( 'sessionid' );
		$whereBaggage = array('sessionId' => $sessionid);
		$whereBaggageOB = array('sessionId' => $sessionid,'type'=>'ob');
		$baggageDataOB = $this->Common_Model->get_all_record_where("*", $whereBaggageOB, "temp_baggage_data");
		$baggagePriceOB = [];
		$baggageWeightOB = [];
		for($i=0; $i<=count($baggageDataOB); $i++){
	   		$baggagePriceOB[] = $baggageDataOB[$i]->baggagePrice;
	   		$baggageWeightOB[] = $baggageDataOB[$i]->baggageWeight;
	   	}
		$totalBaggagePriceOB = array_sum($baggagePriceOB);
		$totalBaggageWeightOB = array_sum($baggageWeightOB);   
	   	$_SESSION ['flight'] [$sessionid] ['flightOB_baggage_price'] = $totalBaggagePriceOB;
	   	$totalWeightOB = array_sum($baggageWeightOB);
	   	$whereBaggageIB = array('sessionId' => $sessionid,'type'=>'ib');
		$baggageDataIB = $this->Common_Model->get_all_record_where("*", $whereBaggageIB, "temp_baggage_data");
		$baggagePriceIB = [];
		$baggageWeightIB = [];
		for($k=0; $k<=count($baggageDataIB); $k++){
	   		$baggagePriceIB[] = $baggageDataIB[$k]->baggagePrice;
	   		$baggageWeightIB[] = $baggageDataIB[$k]->baggageWeight;
	   	}
		$totalBaggagePriceIB = array_sum($baggagePriceIB);
		$totalBaggageWeightIB = array_sum($baggageWeightIB);
		   
	   	$_SESSION ['flight'] [$sessionid] ['flightIB_baggage_price'] = $totalBaggagePriceIB;
	   	$totalWeightIB = array_sum($baggageWeightIB);
		$totalBaggagePrice = ($totalBaggagePriceIB + $totalBaggagePriceOB);
		#########################################################
		$whereMealsOB = array('sessionId' => $sessionid,'type'=>'ob');
		$mealsDataOB = $this->Common_Model->get_all_record_where("*", $whereMealsOB, "temp_meals_data");
		$mealsPriceOB = [];
		for($i=0; $i<=count($mealsDataOB); $i++){
	   		$mealsPriceOB[] = $mealsDataOB[$i]->price;
	   	}
	   	$totalMealsPriceOB = array_sum($mealsPriceOB);
	    $_SESSION ['flight'] [$sessionid] ['flightOB_meal_price'] = $totalMealsPriceOB;
	   	$whereMealsIB = array('sessionId' => $sessionid,'type'=>'ib');
		$mealsDataIB = $this->Common_Model->get_all_record_where("*", $whereMealsIB, "temp_meals_data");
		$mealsPriceIB = [];
		for($k=0; $k<=count($mealsDataIB); $k++){
	   		$mealsPriceIB[] = $mealsDataIB[$k]->price;
	   	}
	   	$totalMealsPriceIB = array_sum($mealsPriceIB);
	   	$_SESSION ['flight'] [$sessionid] ['flightIB_meal_price'] = $totalMealsPriceIB;
		$totalMealsPrice = ($totalMealsPriceIB + $totalMealsPriceOB);
		$_SESSION ['flight'] [$sessionid] ['flight_total_meal_price'] = $totalMealsPrice;
		$bp_user_id = $this->dsa_data->dsa_id;
		$getwayList = $this->Common_Model->get_table_row("*", array(
			"dsapayg_user_type" => "DSA",
			"dsapayg_user_id" => $bp_user_id,
		...
		if ($this->input->post ( 'payment_method' )== "cc_avenue") {
			...
			if(isset($totalBaggagePrice)){
				$baggageFee = ($totalBaggagePrice * $getwayList->dsapayg_convenience_fee) / 100;
				$transaction_fee =  round($transaction_fee + $baggageFee);
				$flight_total_fare = $_SESSION ['flight'][$sessionid] ['flight_total_fare_return'] + $totalBaggagePrice;
				$_SESSION ['flight'] [$sessionid] ['flight_total_fare_return'] = $flight_total_fare;
			}
			if(isset($totalMealsPrice)){
				$mealsFee = ($totalMealsPrice * $getwayList->dsapayg_convenience_fee) / 100;
				$transaction_fee =  round($transaction_fee + $mealsFee);
				$flight_total_fare = $_SESSION ['flight'][$sessionid] ['flight_total_fare_return'] + $totalMealsPrice;
				$_SESSION ['flight'] [$sessionid] ['flight_total_fare_return'] = $flight_total_fare;
			}
		} else {
			$transaction_fee = 0;
		...
		$bookingData['fbook_ib_baggage_amount'] = $totalBaggagePriceIB;
		$bookingData['fbook_ib_baggage_weight'] = $totalBaggageWeightIB;
		$bookingData['fbook_ib_meals_amount'] = $totalMealsPriceIB;
		$bookingData['fbook_ob_baggage_amount'] = $totalBaggagePriceOB;
		$bookingData['fbook_ob_baggage_weight'] = $totalBaggageWeightOB;
		$bookingData['fbook_ob_meals_amount'] = $totalMealsPriceOB;
		...
		foreach ( $FBreakDwn as $val ) {
			...
			for($i = 1; $i <= $noOfPx; $i++) {
				...
				$fpax_baggage_weight = "";
				$fpax_baggage_price = "";				
				$tempBaggageData = $this->Common_Model->get_table_row("*", array(
					"sessionId" => $sessionid,
					"passenger" => $fld_pax_type."_".$i,
				), "temp_baggage_data");
				if($tempBaggageData){
					$fpax_baggage_weight = $tempBaggageData->baggageWeight;
					$fpax_baggage_price = $tempBaggageData->baggagePrice;
				}
				$tempMealsData = $this->Common_Model->get_table_row("*", array(
					"sessionId" => $sessionid,
					"passenger" => $fld_pax_type."_".$i,
				), "temp_meals_data");
				$fpax_meal_item = '';
				$fpax_meal_price = '';
				if($tempBaggageData){
					$fpax_meal_item = $tempMealsData->meal;
					$fpax_meal_price = $tempMealsData->price;
				}
				...
				$bookingPaxDetails = array (
					...
					'fpax_email' => $ci_email,
					'fpax_baggage_weight' =>$fpax_baggage_weight,
	...
	public function save_customer_data() {
		...
		$baggageWeight = [];
		for($i=0; $i<=count($baggageData); $i++){
	   		$baggagePrice[] = isset($baggageData[$i]) ? $baggageData[$i]->baggagePrice : null;
	   		$baggageWeight[] = isset($baggageData[$i]) ? $baggageData[$i]->baggageWeight : null;
	   	}
	   	$totalBaggagePrice = array_sum($baggagePrice);
	   	$totalWeight = array_sum($baggageWeight);
	   	$whereMeal = array('sessionId' => $sessionid,'type'=>'OB');
		$mealData = $this->Common_Model->get_all_record_where("*", $whereMeal, "temp_meals_data");
	   	$mealPrice = [];
	   	for($i=0; $i<=count($mealData); $i++){
	   		$mealPrice[] = isset($mealData[$i]) ? $mealData[$i]->price : null;
	   	}
	   	$totalMealPrice = array_sum($mealPrice);
		foreach($mealData as $meals){
			$mealsamount = $meals->price+$mealsamount;
		}
		$bp_user_id = $this->dsa_data->dsa_id;
		$getwayList = $this->Common_Model->get_table_row("*", array(
			"dsapayg_user_type" => "DSA",
		...
		if ($this->input->post ( 'payment_method' )== "cc_avenue") {
			$transaction_fee = $_SESSION['flight']['Conv_fee'];
			if(isset($baggageData)){
				$baggageFee = ($totalBaggagePrice * $getwayList->dsapayg_convenience_fee) / 100;
				$transaction_fee =  round($transaction_fee + $baggageFee);
				$flight_total_fare = $_SESSION ['flight'] [$sessionid] ['flight_total_fare'] + $totalBaggagePrice;
				$_SESSION ['flight'] [$sessionid] ['flight_total_fare'] = $flight_total_fare;
			}
			if(isset($mealData)){
				$mealsFee = ($totalMealPrice * $getwayList->dsapayg_convenience_fee) / 100;
				$transaction_fee =  round($transaction_fee + $mealsFee);
				$flight_total_fare = $_SESSION ['flight'] [$sessionid] ['flight_total_fare'] + $totalMealPrice;
				$_SESSION ['flight'] [$sessionid] ['flight_total_fare'] = $flight_total_fare;
			}
		} else {
			$transaction_fee = 0;
		...
		if ($searchData ['type'] == "Return" && $searchData ['IsDomestic'] == "true") {
			...
			$complimentoryBaggage = isset($segment[0]->Baggage)  ? $segment[0]->Baggage : 0;
			$totalWeight += (int) $complimentoryBaggage;
			if($searchData ['type'] == "MultiWay"){
			...
			$bookingData = array (
				...
				'fbook_baggage_amount'=>$totalBaggagePrice,
				'fbook_baggage_weight' =>$totalWeight,
				'fbook_meals_amount' =>$totalMealPrice,
	...
	function ticket_lcc($booking_id = NULL, $sessiondata = NULL, $sessionid, $tickettype) {
		...
		$baggageData = $this->Common_Model->get_all_record_where("*", $whereBaggage, "temp_baggage_data");
		$whereMeal = array('sessionId' => $sessionid,'type'=>$tickettype);
		$mealData = $this->Common_Model->get_all_record_where("*", $whereMeal, "temp_meals_data");
	   	$totalPrice = [];
		$passenger = array ();
		$bhanu_i = 0;
			...
			for($i = 1; $i <= $noOfPx; $i ++) {
				...
				if (isset($customer_data['GSTAllowed'])) {
					$bhanu_passenge = array(
						...
						//"Baggage"=>[json_decode($tempBaggageData->rawData)],
						//"MealDynamic" =>[json_decode($tempMealsData->rawData)],
					);
				} else {
					$bhanu_passenge = array (
						"Title" => $customer_data [$fld_pax_type . "Title_" . $i],
				...
				if($tickettype == 'IB'){
			   		unset($bhanu_passenge['MealDynamic']);
			   		unset($bhanu_passenge['Baggage']);	
	   			}

				if(empty($tempMealsData->rawData)){
					unset($bhanu_passenge['MealDynamic']);
				}

				if(empty($tempBaggageData->rawData)){
					unset($bhanu_passenge['Baggage']);
				}
	...
	public function get_booking($booking_id) {	
		...
		if ($Fld_TicketStatus == "Success") {
	        ...
	        if($this->session->userdata('wallet_amount_detected_flight')){
	        	$user_id = $this->session->userdata('Userlogin')["id"];
	            $formData = array (
	                "wallet_id" => $user_id,
	                "order_id" => $booking_id,
	                "customer_id" => $user_id,
	                "amount_detected" => $this->session->userdata('wallet_amount_detected_flight'),
	                "amount_added" => 0,
	                "wallet_action_detail" => 'Flight payment paid by wallet.',
	                "created" => date('y-m-d h:i:s')
	            );
	            $this->VisaModel->insert_table('stm_wallet_history',$formData);
	            $updateArray['cust_balance'] = 0;
	            $this->VisaModel->update_profile($user_id,$updateArray);
	            $this->session->unset_userdata('wallet_amount_detected_flight');
	        }
	...
	public function print_invoice() {
		...
		$data ['PaxDetails'] = $PaxDetail;
		$data ['refId'] = $refId;
		$data['currency'] = (isset($Sel_detail->Response->Response->FlightItinerary->Fare->Currency)) ? $Sel_detail->Response->Response->FlightItinerary->Fare->Currency : "INR";
		if ($Fld_TicketStatus == "Success") {
			if ($Fld_Type == "Return" && $Fld_IsDomestic == "true") {
				$SelectedFareIB = $this->Flight_Model->get_search_detail ( $refId, 'ib_ticket' );
				$Sel_detailIB = json_decode ( $SelectedFareIB->ftemp_data );
	...
	public function booking_error() {
		...
		email_send ( "vishnu.sharma@mailinator.com", "Booking Failed", $message );
	...
	function returnbookingdata($searchData, $sessionid, $cusid, $transaction_fee) {
		...
		$flightOB = ($_SESSION ['flight'] [$sessionid] ['flight_total_fare_OB'] + $_SESSION ['flight'] [$sessionid] ['flightOB_meal_price'] +$_SESSION ['flight'] [$sessionid] ['flightOB_baggage_price'] );
		$flightIB = ($_SESSION ['flight'] [$sessionid] ['flight_total_fare_IB'] + $_SESSION ['flight'] [$sessionid] ['flightIB_meal_price'] +$_SESSION ['flight'] [$sessionid] ['flightIB_baggage_price'] );
		$bookingData = array (
				// 'Fld_RefrenceId' => $ref_id,
			'fbook_total_fare' => str_replace ( ",", "",$flightOB ),
			'fbook_customer_fare' => str_replace ( ",", "", $flightOB ),
			'fbook_ib_total_fare' => str_replace ( ",", "", $flightIB ),
			'fbook_ib_customer_fare' => str_replace ( ",", "", $flightIB ),
			'fbook_booking_type' => $searchData ['type'],
	...
	public function payment_request() {
		$sessionid=$this->input->post("sessionid");	
		$searchData = json_decode($_SESSION ['flight'] [$sessionid]['Search_data_json']);		
		$currencySelected = $searchData->PreferredCurrency;
        ...
        $wallet_amt = $this->user_data->cust_balance;
	...
	private function loadAEDPaymentTransaction($data){
		...
		$dataJson = array (
			...
			"redirecturl" => site_url () . "flight/payment_payd_response"
		);
	...
	public function payment_payd_response(){
		...
		if($ref_id){
			...
			$bookingId = isset($transaction['gateway_data'] ["order_id"]) ?$transaction['gateway_data'] ["order_id"] : 0;
			$_SESSION['flight']['search_RequestData']["BookingId"] = $bookingId ;	
			return redirect ( '/flight/payment_error/' );
		}
	}
	...
	public function payment_result() {
		...
		$status = $order_status;
		$BookingId = $booking_id;
		$data = array ('fbook_payment_status' => $status );
		$this->Flight_Model->update_booking ( $data, $BookingId );
		$paymentResponse = json_encode ( $_POST );
		$search_data = array (
			'search_type' => 'flight',
