...
function insert_pax_offline($data){
	...
	public function getHotelTransaction($bookingId){
		$this->db->select("*");
		$this->db->where('booking_id', $bookingId);
		$query = $this->db->get('hotel_payment_history');
		if($query->num_rows() > 0){
			return $query->row();
		}else{
			return false;
	...
	public function addInitialTransaction($data){
        $this->db->insert('hotel_payment_history', $data);
        return $this->db->insert_id();
    }
    
    public function updatePaymentTransaction($data){
        $transactionData = $this->getHotelTransaction($data['booking_id']);
        $this->db->where('id', $transactionData->id);
        $this->db->update('hotel_payment_history', $data);
        return $transactionData->id;
    }

	public function addHotelPayment($data){
        $transactionData = $this->getHotelTransaction($data['order_id']);
        $formData = array (
