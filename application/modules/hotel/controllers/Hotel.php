...
class Hotel extends MX_Controller {
	...
	public function result() {
		if (isset ( $_SESSION ['hotel'] ['array'] ['search_result'] )) {
			if ($_SESSION ['hotel'] ['array'] ['search_result']->HotelSearchResult->Error->ErrorCode == "0") {
				...
				// PrintArray(json_decode($_SESSION ['hotels']));
	...
	public function resultCard($results, $key = '') {
		...
		$html = '<div class="rating_str card_'.$key.'" rating="' . $results->StarRating . '">
				                  ...
				                  <div class="col-lg-2 col-md-2 col-sm-2 col-xs-12">
				                    <div class="htl-listing-price text-center">
				                      ...
				                      <div class="htl-view-list">
										 <form  action="' . site_url () . 'hotel/booking_detail" method="POST" target="_blank">
	...
	public function block_room() {
		...
		foreach ( $bp_room_data as $bp_room_datas ) {
			if (in_array ( $bp_room_datas->RoomIndex, $bp_selected_room_index )) {
				...
				$bp_select_room_array ["RatePlanName"] = $bp_room_datas->RatePlanName;
				...
				$bp_select_room_array ["BedTypeCode"] = $bp_room_datas->BedTypeCode;
				$bp_select_room_array ["BedTypeDescription"] = "";
				$bp_select_room_array ["SmokingPreference"] = "0";
				$bp_select_room_array ["Supplements"] = NULL;
				$bp_select_room_array ["Price"]['CurrencyCode'] = $bp_room_datas->Price->CurrencyCode;
				$bp_select_room_array ["Price"]['RoomPrice'] = getDecimal($bp_room_datas->Price->RoomPrice); 
				$bp_select_room_array ["Price"]['Tax'] = getDecimal($bp_room_datas->Price->Tax);
				$bp_select_room_array ["Price"]['ExtraGuestCharge'] = $bp_room_datas->Price->ExtraGuestCharge;
				$bp_select_room_array ["Price"]['ChildCharge'] = $bp_room_datas->Price->ChildCharge;
				$bp_select_room_array ["Price"]['OtherCharges'] = $bp_room_datas->Price->OtherCharges;
				$bp_select_room_array ["Price"]['Discount'] = $bp_room_datas->Price->Discount;
				$bp_select_room_array ["Price"]['PublishedPrice'] = getDecimal($bp_room_datas->Price->PublishedPrice);
				$bp_select_room_array ["Price"]['PublishedPriceRoundedOff'] = $bp_room_datas->Price->PublishedPriceRoundedOff;
				$bp_select_room_array ["Price"]['OfferedPrice'] = getDecimal($bp_room_datas->Price->OfferedPrice);
				$bp_select_room_array ["Price"]['OfferedPriceRoundedOff'] = $bp_room_datas->Price->OfferedPriceRoundedOff;
				$bp_select_room_array ["Price"]['AgentCommission'] = getDecimal($bp_room_datas->Price->AgentCommission);
				$bp_select_room_array ["Price"]['AgentMarkUp'] = $bp_room_datas->Price->AgentMarkUp;
		...
		if (isset ( $_SESSION ['hotel'] ['array'] ['search_request'] ['RoomGuests'] )) {
			...
			foreach ( $bp_pax_request as $bp_pax_key => $bp_pax_requests ) {
				for($i = 0; $i < $bp_pax_requests ['NoOfAdults']; $i ++) {
					...
					$bp_pax_array ["PassportNo"] = $this->input->post("passport_adult_".$bp_pax_key. "_".$i) ? $this->input->post ( "passport_adult_" . $bp_pax_key . "_" . $i ) : null;
					$bp_pax_array ["PassportIssueDate"] = null;
					$bp_pax_array ["PassportExpDate"] = $this->input->post ("passport_exp_adult_".$bp_pax_key."_".$i) ? $this->input->post ( "passport_exp_adult_" . $bp_pax_key . "_" . $i ) : null;;
					$bp_pax_array ["PAN"] = $this->input->post("pan_adult_".$bp_pax_key."_".$i) ? $this->input->post("pan_adult_" . $bp_pax_key . "_" . $i ) : null;;
	...
	private function generateOrderData($data){
        $this->session->unset_userdata('hotelTransactionData');
        $orderData = [
            "amount" => $data['amount']*100,
            "currency" => "INR",
            "receipt" => "hotel_".$data['order_id']
        ];
	...
	private function storeTransactionData($paymentData,$orderData){
        ...
        $isExist = $this->Hotel_Model->getHotelTransaction($dbData['booking_id']);
        if($isExist){
            $this->Hotel_Model->updatePaymentTransaction($dbData);
        }else{
            $this->Hotel_Model->addInitialTransaction($dbData);
        }
    }

	public function handleResponse(){
        $response = ['status' => 'failed','route' => 'visa'];
        if($this->session->has_userdata('hotelTransactionData')){
            $hotelTempData = $this->session->userdata('hotelTransactionData');
            $status = $_POST['status'];
            $responseData = $_POST['response'];
            if($status == "success" && $responseData['razorpay_order_id'] == $hotelTempData['gatewayData']['id']){
                $hotelTempData['paymentData']['tracking_id'] = $responseData['razorpay_order_id'];
				$hotelTempData['paymentData']['amount'] = $hotelTempData['gateway_data']['amount'];
				$hotelTempData['paymentData']['order_id'] = $hotelTempData['gateway_data']['order_id'];
                $hotelTempData['paymentData']['order_status'] = "Success";
                $hotelTempData['paymentData']['discount_value'] = 0;
                $hotelTempData['paymentData']['type'] = "razor_pay";
                $hotelTempData['paymentData']['razor_pay_response'] = json_encode($responseData);
                $hotelTempData['paymentData']['trans_date'] = date("d/m/Y H:m:i");
				$this->Hotel_Model->addHotelPayment($hotelTempData['paymentData']);
                $this->session->unset_userdata('hotelTransactionData');
                $this->session->set_flashdata('flash_success','Thank you for booking hotel room. Your transaction is successful.');
                $response['status'] = 'success';
				$BookingId = isset($hotelTempData['gateway_data'] ["order_id"]) ?$hotelTempData['gateway_data'] ["order_id"] : 0;
				$data = array (
					'hotboli_payment_status' => $orderStatus,
					'hotboli_booking_status' => $orderStatus, 
				);
				$this->Hotel_Model->update_booking ( $data, $BookingId );
				$response['route'] = '/hotel/book/';	
			}else{
				$bookingId = isset($hotelTempData['gateway_data'] ["order_id"]) ?$hotelTempData['gateway_data'] ["order_id"] : 0;
				$_SESSION['flight']['search_RequestData']["BookingId"] = $bookingId ;
				$hotelTempData['paymentData']['tracking_id'] = $hotelTempData['gatewayData']['id'];
                $hotelTempData['paymentData']['order_status'] = "Failed";
				$hotelTempData['paymentData']['amount'] = $hotelTempData['gateway_data']['amount'];
				$hotelTempData['paymentData']['order_id'] = $hotelTempData['gateway_data']['order_id'];
                $hotelTempData['paymentData']['discount_value'] = 0;
                $hotelTempData['paymentData']['type'] = "razor_pay";
				...
				$this->Hotel_Model->addHotelPayment($hotelTempData['paymentData']);
                $this->session->unset_userdata('hotelTransactionData');
                $this->session->set_flashdata('flash_error','Your hotel booking transaction got failed. Please try again.');
                $response['status'] = 'error';
                $response['route'] = '/hotel/payment_error/';	
			}
        }
