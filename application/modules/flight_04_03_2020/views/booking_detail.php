	...
	<div class="container">
		<div class="row">
			<div class="col-md-9">
							...
							<div class="flt-booking-dts">
								<div class="col-fly-inn">
									 ...
									 <?php }?>
					                    ...
					                    <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
					                      <div class="form-group">
					                        <input type="text" error_msg="Please select passport Exp. for <?php echo $PassengerType; ?>"  name="<?php echo $PassengerType; ?>PassExpDate_<?php echo $i; ?>" id="pass-exp" readonly="" placeholder="Passport Exp." class="form-control co_dateofbirth ex_date pax_validation_field co_expdatebirth">
					                      </div>
					                    </div>
					                    <div class="col-lg-3 col-md-3 col-sm-6 col-xs-12">
				...
				<div class="flt-booking-wrap mb-2 mb-md-3">
						<div class="flt-booking-top">
							<h5><span>Baggage</span> <span class="float-right">Meal & Bavrage</span></h5>
						</div>
						<div class="flt-booking-dts">
							<div class="row">
								<div class="col-md-6">
									 <div class="form-group">
									 	<select class="form-control" name="baggage" id="baggageFare">
									 	<?php for($i=0; $i<count($baggage->Response->Baggage[0]); $i++){ 
									 		
									 		?>
									 			<option value="<?php echo ($baggage->Response->Baggage[0][$i]->Weight); ?>"><?php echo ($baggage->Response->Baggage[0][$i]->Weight); ?> Kilogram</option>
									 		<?php } ?>	
									 	</select>
									 </div>
								</div>
...
<div class="multiple_select_checkbox">
    ...
    <?php for($i=0; $i<count($baggage->Response->MealDynamic[0]); $i++){ 
				if(!empty($baggage->Response->MealDynamic[0][$i]->AirlineDescription)){
	?>

   <div class="multiple_select_checkbox_choice"><input type="checkbox" name="meal[]" class="mealcheckbox" value="<?php echo base64_encode(($baggage->Response->MealDynamic[0][$i]->AirlineDescription."#".$baggage->Response->MealDynamic[0][$i]->Price.'#'.$baggage->Response->MealDynamic[0][$i]->Code)); ?>" /><?php echo ($baggage->Response->MealDynamic[0][$i]->AirlineDescription); ?></div>
				...
				<div class="flght-side-det">
		            ...
		            <div class="contant">
		            	<ul class="fare_details list-unstyled mb-0">
		            		...
		            		<li>
								<a class="collapsed" href="javascript:void(0)" role="button" data-toggle="collapse" data-target="#basefare">Base Fare<small>(<?php echo $total_pax; ?> Traveller) </small> <i class="icofont-rounded-down"></i></a><span class="float-right"><i class="icofont-rupee"></i> <?php echo round($base_fare); ?></span>
									...
									<?php
									  ...
									  $noOfPess = $FBDetails->PassengerCount;
										...
										<li>
											...
											<span class="float-right"><i class="icofont-rupee"></i> <?php echo round($FBDetails->BaseFare); ?></span>
										</li>
										  <?php } } ?>	
		            		...
		            		<!--<li>Base Fare <span class="float-right"><i class="icofont-rupee"></i> <?php echo ($base_fare); ?></span></li>-->
		            		<li>Tax (+)<span class="float-right"><i class="icofont-rupee"></i> <?php echo round($tax) ?></span></li>
						 
							<li>Convenience Fee (+)<span class="float-right"><i class="icofont-rupee"></i> <span class="conv totalConvenienceFee"> <?php echo $final_con; ?></span></span></li>
							...
							<?php  if($this->session->userdata("Userlogin") != NULL && $this->user_data->cust_balance > 0 && $wallet[0]->detected_wallet_percentage > 0 && $wallet[0]->detected_wallet_percentage < 100){ ?> 
			            		<li>Wallet Amount (-)<span class="float-right"><i class="icofont-rupee"></i> <?php echo ($this->user_data->cust_balance * $wallet[0]->detected_wallet_percentage) / 100; ?></span></li>
			            	<?php } ?>

			            	
							<li>Baggage <span class="baggage-weight"></span> <span class="float-right"><i class="icofont-rupee"></i><span class="conv baggage-price"></span></span></li>

							<li>Meal  & Bavarage
								<span class="list-bavarage"></span>
							</li>
						   ...
						   ?>
						   <!--
						   <li>Convenience Fee<span class="float-right"><i class="icofont-rupee"></i><span class="gateway_charge"> <?php echo $nkwithconfee; ?> <span></span></li>
						   -->
						  <?php } else { $nkwithconfee = 0; } ?> 
						   <li style="display: none;" class="coupon none"></li>
		            		<li>Total Fare<span class="float-right"><i class="icofont-rupee"></i><span class="final_fare finalamount"><?php echo round($customer_fare + $final_con); ?></span></span></li>
		            	</ul>
		            </div>
...
//     		console.log(data);
//     		$(".baggage-weight").html(data.Weight+'KG');
//     		$(".baggage-price").html(data.Price);
//     		localStorage.setItem("baggageitem",data.Price);
//     		let totalItemAmount = convFee + parseInt(localStorage.getItem("baggageitem"));
//     		let totalConvenienceFee = (totalItemAmount * <?php echo $getwayList->dsapayg_convenience_fee ?>) / 100;
// 			$(".totalConvenienceFee").html(Math.round(totalConvenienceFee));
//     		$(".finalamount").html(Math.round(totalItemAmount+totalConvenienceFee)); 
//     		localStorage.setItem("convenienceAmount",totalConvenienceFee);
//     		localStorage.setItem("totalFare",Math.round(totalItemAmount+totalConvenienceFee));
    		

//     	}  
// 		})
// 	}else{
// 		localStorage.setItem("convenienceAmount",<?php echo $final_con; ?>);
//     	localStorage.setItem("totalFare",<?php echo round($customer_fare + $final_con); ?>);
// 	}
...
$(':checkbox[class=mealcheckbox]').on('click', function() {
	let convFee = <?php echo $DirPublishedFarePrice; ?>;
    var assignedTo = $(':checkbox[class=mealcheckbox]:checked').map(function() {
    ...
    var mealPrice=[];
    var sum=0;
    assignedTo.forEach(function (item, index) {
    	var keywords = item.split("#");

    	mealPrice[index]=parseInt(keywords[1],0);
    	getmeal[index] = keywords[2];
  		list+='<li>'+keywords[0]+' <span class="baggage-weight"></span> <span class="float-right"><i class="icofont-rupee"></i><span class="conv meal-price">'+keywords[1]+'</span></span></li>';
	});
	localStorage.setItem("mealDynamic",getmeal);
	$(".list-bavarage").html(list);
    for(var i = 0; i < mealPrice.length; i++){
		sum += parseInt(mealPrice[i],0);
	}
	
	localStorage.setItem("totalMeal", sum);
	
	let totalItemAmount = convFee + parseInt(localStorage.getItem("baggageitem")) + parseInt(localStorage.getItem("totalMeal"));
...
//script for login
$("#baggageFare").on("change",function(){
	let baggage = $("#baggageFare").val();
	let baggageJson = <?php echo json_encode($baggage->Response->Baggage[0]); ?>;
	let convFee = <?php echo $DirPublishedFarePrice; ?>;
...
$("#mealFare").on("change",function(){
	let meal = $("#mealFare").val();
	let mealJson = <?php echo json_encode($baggage->Response->MealDynamic[0]); ?>;

	$.each(mealJson, function (key, data) {
    	if(data.AirlineDescription == mealJson){
    		$(".meal-description").html(data.AirlineDescription);
    		$(".meal-price").html(data.Price);

    		$(".Mealfinalamount").html(<?php echo round($customer_fare + $final_con); ?>+data.Price);
				...
				$(function(){
					$(".proceed_payment_data").click(function(){
						$("#form_submit_pop_over").modal("show");
    						...
    						localStorage.getItem("mealDynamic");

							var convenienceAmount = {'name': "convenienceAmount", 'value': localStorage.getItem("convenienceAmount")};
							//var totalFare = {'name': "totalFare", 'value': localStorage.getItem("totalFare")};

							//let totalItemAmount = <?php echo $DirPublishedFarePrice; ?> + parseInt(localStorage.getItem("baggageitem"));
							
							var farequote = {'name': "farequote", 'value': localStorage.getItem("totalFare")};
							
							//let mealDynamic = {'name':"mealDynamic",'value':localStorage.getItem("mealDynamic")}
							
							formD.push(convenienceAmount);
							//formD.push(totalFare);
							//formD.push(baggage);
							formD.push(farequote);
