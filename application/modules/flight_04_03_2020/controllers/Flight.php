...
class Flight extends MX_Controller {
	...
	public function booking_detail() {
		...
		// echo "<pre>";
		 ...
		 // print_r($mealDynamicResult->Response->Baggage[0]);
		 // die;
	...
	public function save_customer_data() {
		...
		if ($this->input->post ( 'payment_method' )== "cc_avenue") {
			...
			$transaction_fee = round($this->input->post ( 'convenienceAmount' ));
		} else {
			$transaction_fee = 0;
		}
		

		// if($this->input->post ( 'mealDynamic' )){
		// 	$_SESSION ['flight'] [$sessionid] ['search_RequestData']['mealDynamic'] = $this->input->post ( 'mealDynamic' );	
		// }
		

		 //print_r($transaction_fee);
		 //die;
		if ($searchData ['type'] == "Return" && $searchData ['IsDomestic'] == "true") {
			...
			$_SESSION ['flight'] [$sessionid]['flight_total_fare'] = $this->input->post ( 'farequote' );
			
			//print_r($this->input->post ( 'farequote' )); die;
			...
			for($i = 1; $i <= $noOfPx; $i ++) {
				...
				if (isset($customer_data['GSTAllowed'])) {
					$bhanu_passenge = array(
						...
						"Baggage"=>(isset($_SESSION ['flight'] [$sessionid] ['search_RequestData']['baggage']) ? $_SESSION ['flight'] [$sessionid] ['search_RequestData']['baggage'] : ''),
						"MealDynamic" =>(isset($_SESSION['flight'] [$sessionid] ['search_RequestData'] ['mealDynamic']) ? $_SESSION['flight'] [$sessionid] ['search_RequestData'] ['mealDynamic'] : '')
...
//TICKET GDS
	...
	public function payment_request() {
		...
		//print_r($mealDynamicResult->Response->MealDynamic[0]);
		$meals = [];
		foreach($_REQUEST['meal'] as $meal){
			 $keywords = explode('#',base64_decode($meal));
			 //print_r($mealDynamicResult->Response->MealDynamic[0]);
			 
			 foreach($mealDynamicResult->Response->MealDynamic[0] as $key=>$meal){
			 	
			 	if($keywords[0] == $meal->AirlineDescription){
			 		$meals[] = $meal;
			 	}
			 	
			 }
		} 
		$_SESSION['flight'] [$sessionid] ['search_RequestData'] ['mealDynamic'] = $meals;
		
		foreach($mealDynamicResult->Response->Baggage[0] as $bagg){
			if($bagg->Weight == $_REQUEST['baggage']){
				$_SESSION ['flight'] [$sessionid] ['search_RequestData']['baggage'] = $bagg;
				$baggagePrice = $bagg->Price;
			}
		}
