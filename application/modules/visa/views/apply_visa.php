					...
					<div class="row">
									...
									<?php if(!empty($visalist)) { 
										...
										foreach($visalist as $visadata){ 
										  if(isset($visa_id) && $visa_id == $visadata->id) {
											...
											$visaDuration = $visadata->duration;
										  } else {
										    $selected = ''; 
										  }
										  ?>
						...
						<div class="col-md-12">
							<div class="checkbox htl-srch-rtng">
								<label for="term_accept" class="accept">
									<input type="checkbox" id="term_accept" class="flightfare" value="1" name="terms">
									<span>
									Visa approval is subject to Immigration approval and all visa fees are non-refundable
									</span>
								</label>
							</div>
						</div>
						<?php 
							...
							<div class="col-md-12">
								<div class="checkbox htl-srch-rtng">
									<label for="user_wallet" class="accept">
										<input name="payment_method" id="user_wallet" class="term_condition" type="checkbox" value="customerCash"> 
										<span>
											Use Customer Wallet Amount <b>Available Balance is: </b><i class="icofont-rupee"></i><?php echo $bp_customer_total_balance; ?>
										</span>
									</label>	
								</div>
							</div>
...
<script type="text/javascript">
	$(document).ready(function(){
		var durationDays = parseInt("<?= $visaDuration ?>");
		...
		$('#departure-date').datepicker({
			...
			onClose: function (selectedDate){
				...
				var today = new Date();
				data.setDate((today.getDate()+durationDays));
				var dd = data.getDate();
				var mm = data.getMonth() + 1;
				var y = data.getFullYear();
				var formatedDate = y+"-"+mm+"-"+dd;
				console.log(formatedDate);
				$("#arrivalDate").datepicker("option",{
		    ...
		    console.log(e);
		    var selectedDate = $('#departure-date').val();
		    var data = new Date(selectedDate);
			var today = new Date();
			data.setDate((today.getDate()+durationDays));
			var dd = data.getDate();
			...
			var y = data.getFullYear();
			var formatedDate = y+"-"+mm+"-"+dd;
            $('#arrivalDate').datepicker('setStartDate', selectedDate);
            $('#arrivalDate').datepicker('setEndDate', formatedDate);
        });
	...
	$(document).on("form.frmTransaction").on("submit",function(e){
